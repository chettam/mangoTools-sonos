...
const appleDef = require('../services/appleDef');
const spotifyDef = require('../services/spotifyDef');
const deezerDef = require('../services/deezerDef');
const eliteDef = deezerDef.init(true);
const libraryDef = require('../services/libraryDef');

const musicServices = ['apple','spotify','deezer','elite','library'];
const musicTypes = ['album','song','station','load','setsid'];
const sidsPath = "./lib/sids.json";
...
function handleSids(err, data) {
    if (!err) {
        var storedIds = JSON.parse(data);
        Object.keys(storedIds).map(function(service) {
            serviceIds[service] = storedIds[service];
        });
    }
}

function setSid(player, service, song) {
    console.log("Setting Service ID for " + service);

    return player.browse('FV:2', 0, 0)
            .then(result => result.items)
.then((favorites) => {
        return favorites.find((fav) => fav.title.toLowerCase() === song.toLowerCase());
})
.then((favorite) => {
        if (!favorite) {
        console.log("The song " + song + " was not found in Sonos Favorites");
        return Promise.reject("The song " + song + " was not found in Sonos Favorites");
    }

    var sidLoc = favorite.uri.indexOf('sid=');

    if (sidLoc == -1) {
        console.log(song + " in Sonos Favorites needs to be a song type");
        return Promise.reject(song + " in Sonos Favorites needs to be a song type");
    }
    return favorite.uri.substring(sidLoc+4,favorite.uri.indexOf('&', sidLoc));
})
.then((sid) => {
        serviceIds[service] = sid;
    return new Promise((resolve, reject) => {
            fs.writeFile(sidsPath,JSON.stringify(serviceIds),(err) => {
            if (err) {
                return reject(err);
            } else {
                return resolve();
...
function getAccountId(player, service)
{
    ...
    if (service != 'library') {
        return request({url: player.baseUrl + '/status/accounts',json: false})
                .then((res) => {
                var actLoc = res.indexOf(serviceIds[service] * 256 + 7);
...
function doSearch(service, type, term)
{
    ...
    if (service == 'library') {
        return Promise.resolve(libraryDef.search(type, newTerm));
    } else
...
function musicSearch(player, values) {
    ...
    if ((service != 'library') && (type == 'setsid')) {
        return setSid(player, service, term);
    }

    return getAccountId(player, service)
        .then(() => {
        serviceDef = getService(service);
    serviceDef.service(serviceIds[service], accountId, accountSN, country);
...
module.exports = function (api) {
    ...
    libraryDef.read();
    fs.readFile(sidsPath, handleSids);
};/**
 * Created by jbblanc on 20/09/2016.
 */
